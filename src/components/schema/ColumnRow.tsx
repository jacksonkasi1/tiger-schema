      ...
      <Popover open={typeOpen} onOpenChange={setTypeOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            className="h-8 flex-[0.8] min-w-[70px] justify-between text-xs font-mono px-2 text-muted-foreground hover:text-foreground hover:bg-muted/40 transition-all duration-150 rounded"
          >
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-52 p-1.5">
            ...
            >
              ...
              {indexType === 'primary_key' && <Check className="h-4 w-4 text-amber-500" />}
            </DropdownMenuItem>
            <DropdownMenuItem
            ...
            >
              ...
              {indexType === 'unique_key' && <Check className="h-4 w-4 text-indigo-500" />}
            </DropdownMenuItem>
            <DropdownMenuItem
            ...
            >
              ...
              {indexType === 'index' && <Check className="h-4 w-4 text-violet-500" />}
            </DropdownMenuItem>
            <DropdownMenuItem
            ...
            >
              ...
              {indexType === 'none' && <Check className="h-4 w-4 text-muted-foreground" />}
            </DropdownMenuItem>

            <DropdownMenuSeparator className="my-1.5" />
            {/* FK Selector in same menu */}
            <div className="px-2 py-1.5">
              <p className="text-xs font-medium text-muted-foreground mb-2">Foreign Key</p>
              {column.fk ? (
                <div className="flex items-center justify-between">
                  <span className="text-xs font-mono text-emerald-600 dark:text-emerald-400">{column.fk}</span>
                  <button
                    onClick={() => updateColumn(tableId, columnIndex, { fk: undefined })}
                ...
                <Popover open={fkOpen} onOpenChange={setFkOpen}>
                  ...
                  <PopoverContent className="w-[280px] p-0 border-border/60 shadow-xl" align="end" side="right">
                    <Command className="bg-popover">
                      <CommandInput
                      ...
                      <CommandList className="max-h-[240px] p-1">
                        <CommandEmpty className="py-4 text-xs text-muted-foreground/60 text-center">No columns found.</CommandEmpty>
                        {Object.entries(fkOptionsGrouped).map(([tableName, columns]) => (
                          <CommandGroup key={tableName} heading={tableName} className="py-0.5">
                            {columns.map((option) => (
                              <CommandItem
                                key={option.value}
                                value={option.value}
